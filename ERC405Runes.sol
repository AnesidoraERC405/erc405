...
abstract contract ERC405 is ERC20, ERC1155 {
    ...
    uint256 private _runseTotalSupply;
    uint256 private _runseTotalMinted;
    uint256 private _runseId = 405;
    ...
    constructor(
        ...
        ERC20(
            ...
            symbol,
            initialDecimals,
            initialConversionRate * runseSupply
        )
    {
        _conversionRate = initialConversionRate;
        _runseTotalSupply = runseSupply;
    ...
    function safeMint(uint256 amount) internal virtual {
        if (totalMinted() + amount > _runseTotalSupply) {
            revert TotalSupplyExceeded();
        }
        _runseTotalMinted += amount;
        ERC1155.mint(msg.sender, _runseId, amount);
    ...
    function safeBatchTransferFrom(
        ...
        if (ids.length > 1 || ids[0] != _runseId) {
            revert InsufficientBalance();
        }
    ...
    function _checkAndTransferNFT(
        ...
        uint256 _nftSenderBalance = ERC1155.balanceOf(sender, _runseId);
        uint256 _erc20TokenSenderBalance = ERC20.balanceOf(sender);

        uint256 _nftReceiverBalance = ERC1155.balanceOf(receiver, _runseId);
        ...
        if (
            ...
            uint256 _nftThisBalance = ERC1155.balanceOf(address(this), _runseId);
            ...
            if (_nftThisBalance != 0) {
                ...
                ERC1155.mint(receiver, _runseId, _receiverAddNFT);
            }
        }
        return true;
    }
    ...
    function _burn(uint256 amount) internal virtual {
        ...
        uint256 _nftSenderBalance = ERC1155.balanceOf(msg.sender, _runseId);
